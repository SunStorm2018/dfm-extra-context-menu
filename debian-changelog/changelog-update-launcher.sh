...
# 注意：安装后脚本路径为 /usr/share/dde-file-manager-extramenu-plugins/debian-version-update.sh
if [ -f "/usr/share/dde-file-manager-extramenu-plugins/debian-version-update.sh" ]; then
    exec "/usr/share/dde-file-manager-extramenu-plugins/debian-version-update.sh" "$@"
else
    # 开发环境下的路径
